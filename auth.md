  ...
  <body class="bg-slate-50 text-slate-900">
    ...
    <script>
      ...
      qs("#btnCreateEuk").addEventListener("click", async () => {
        const fd = Object.fromEntries(new FormData(qs("#formEuk")).entries());
      ...
      qs("#btnCreateLsp").addEventListener("click", async () => {
        const fd = Object.fromEntries(new FormData(qs("#formLsp")).entries());
        delete fd.id;
      ...
      qs("#btnCreateScheme").addEventListener("click", async () => {
        const fd = Object.fromEntries(
          new FormData(qs("#formScheme")).entries()
        );
        delete fd.id;
        fd.isActive = !!qs("#formScheme").isActive?.checked;
      ...
      qs("#btnCreateJuk").addEventListener("click", async () => {
        const fd = Object.fromEntries(new FormData(qs("#formJuk")).entries());
        delete fd.id;
        // Convert numeric fields from string if necessary, though fetch handles string body well
      ...
      qs("#fillJukDummy").addEventListener("click", () => {
        const f = qs("#formJuk");
        f.id.value = "";
        f.judulKegiatan.value = "JUK Skema A - " + Math.random().toString(36).substring(7).toUpperCase();
        f.tanggalPelaksanaan.value = "2024-11-15";
        f.jamPelaksanaan.value = "09:00 - 16:00";
        f.kuotaPeserta.value = 20;
        f.schemeId.value = 1; // Assuming Scheme ID 1 exists
        f.tukId.value = 1; // Assuming TUK ID 1 exists
        f.asesorId.value = 3; // Assuming Asesor User ID 3 exists
      ...
      qs("#btnCreateTuk").addEventListener("click", async () => {
        const fd = Object.fromEntries(new FormData(qs("#formTuk")).entries());
        delete fd.id;
      ...
      qs("#fillTukDummy").addEventListener("click", () => {
        ...
        f.lspIndukId.value = 1; // Assuming LSP ID 1 exists
